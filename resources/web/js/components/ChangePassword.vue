<template>
    <div class="flex">
        <link rel="stylesheet" :href="baseUrl+'public/admin/css/site.min.css'" />
        <div class="spinner-border" role="status" v-if="resetPwdTokens.length == 0">
            <span class="sr-only ">Loading...</span>
        </div>
        <div class="w-xl w-auto-sm mx-auto pt-5" v-else>
            ...
            <div class="card">
                <div id="content-body">
                    <div class="p-3 p-md-4">
                        ...
                        <p><small class="text-muted">Change password</small></p>
                        <form class="" role="form">
                            <div class="form-group">
                                <label class="text-muted text-capitalize">New password</label>
                                <input v-model="forgotPassword.password" :class="status($v.forgotPassword.password)" type="password" class="form-control dv_common_input_for_all" placeholder="Enter new password" />
                                <span class="text-danger" v-if="getError.hasOwnProperty('password')">{{ getError['password'][0] }}</span>
                            </div>
                            <div class="form-group">
                                <label class="text-muted text-capitalize">Repeat password</label>
                                <input v-model="forgotPassword.password_confirmation" :class="status($v.forgotPassword.password_confirmation)" type="password" class="form-control  dv_common_input_for_all" placeholder="Repeat password" />
                            </div>
                            <button type="button" class="btn btn-primary mb-4 dv_login_btn" @click="submit">Change Password</button>
                        </form>
                    </div>
                </div>
            </div>
...
import { FORGOT_PASSWORD_ACTION, GET_RESET_PASSWORD_TOKEN_ACTION  } from '@web/store/users/actions'
...
export default {
    ...
    computed:{
        ...mapState({
            ...
            resetPwdTokens: state => state.Users.getResetPasswordToken,
        })
    },
    mounted() {
        this.getResetPwdTokens();
    },
    methods: {
        status(validation) {
            return {
        ...
        getResetPwdTokens: function() {
            let query = [];
            query['params'] = { token: this.$route.params.token };
            this.$store.dispatch('Users/' + GET_RESET_PASSWORD_TOKEN_ACTION, query);
        },
        submit: function() {
            let _this = this;
            _this.$v.forgotPassword.$touch();
            if (!_this.$v.forgotPassword.$invalid) {
                _this.forgotPassword['token'] = _this.$route.params.token;
                _this.$store.dispatch('Users/' + FORGOT_PASSWORD_ACTION,_this.forgotPassword).then(() => {
