					...
					<div class="row mb-3">
						<div class="col-md-12">
							<div class="dv_confirmed_placed_card">
								...
								<h4>Order succesfully placed </h4>
								<p>Thank you for your order {{ getUser.first_name }}!</p>
					...
					<div class="row">
						...
						<div class="col-md-4" v-if="getPayment != null">
							...
							<div class="dv_order_summery_detail">
								...
								<span>vat <strong>aed {{ $attrs.getTotal.vat }}</strong> </span>
								<span>shipping <strong>aed {{ getPayment.shipping }}</strong> </span>
								<span>delivery <strong>aed {{ getPayment.delivery_charge }}</strong> </span>
								<span v-if="$attrs.getCarts['coupon_id']" >Coupon ({{ $attrs.getCarts['get_coupon']['code'] }}) <strong>aed {{ getOrder.get_discount.coupon_discount_amount }}</strong> </span>
								<span v-if="$attrs.getCarts['gift_certificate_id']">Gift Voucher ({{ $attrs.getCarts['get_gift_certificate']['certificate_code'] }}) <strong>aed {{ getOrder.get_discount.gift_discount_amount }}</strong> </span>
						...
						<div class="col-md-8">
							...
							<div class="dv_per_product_row" v-for="(item,index) in getOrderItems">
...
<script type="text/javascript">
 ...
 import { SAVE_GET_ORDER_ACTION } from '@web/store/orderConfirms/actions'
 export default {
 	 components: {  },
 	  data() {
            return {
		...
		methods: {
         	continueShop:function(){
				window.location.href = this.baseUrl + 'get-product-list/ae-en';
         	},
            ...
            getOrderFunc:function(order){
         	...
         	getOrderConfirm:function(paymentMode){
         		let _this = this;
            	if(!Vue.helpers.isEmpty(_this.$user) && Vue.helpers.getSession() != null){
	            	_this.$store.dispatch('OrderConfirm/'+SAVE_GET_ORDER_ACTION, Vue.helpers.getSession()).then(function(){
            		...
            		_this.$router.push({name:'getOrders'}) 
            	}
         	},
