                    ...
                    <div id="content-body">
                        <div class="p-3 p-md-4">
                            ...
                            <form class="" role="form">
                                <div class="form-group"><label class="text-muted text-capitalize">Email</label><input type="text" class="form-control dv_common_input_for_all" placeholder="Enter email" v-model="login.email" :class="status($v.login.email)" @input="setLoginEmail($event.target.value)"/></div>
                                <div class="form-group">
                                    <label class="text-muted text-capitalize">Password</label><input type="password" class="form-control  dv_common_input_for_all" placeholder="Password" v-model="login.password" :class="status($v.login.password)" @input="setLoginPassword($event.target.value)"/>
                                </div>
                               <p class="typo__p" v-if="loginSubmitStatus === 'OK'">Thanks for your submission!</p>
                                ...
                                <button type="button" class="btn btn-primary mb-4 dv_login_btn"  @click="getSignIn"><!-- <span class="spinner-border spinner-border-sm mr-3"></span> -->Sign in</button>
                                <div class="my-3 text-center"><router-link :to="{ name: 'forgotPassword' }" class="ml-1"><strong>Forgot password?</strong></router-link></div>
                            </form>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
 ...
 import { LOAD_LOGIN_ACTION,LOAD_USER_ACTION,SAVE_USER_REGISTER_ACTION } from '@admin/store/users/actions'
