    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">Edit Stock Transfer </h2>
                        ...
                        <div class="col-md-12">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            <div class="form-group">
                                                ...
                                                <select name="from_location" v-model="saveStockTransfer.from_location" :class="status($v.saveStockTransfer.from_location)" @input="setValue($event)" class="form-control dv_common_select_for_all">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            <div class="form-group">
                                                ...
                                                <select name="to_location" v-model="saveStockTransfer.to_location" :class="status($v.saveStockTransfer.to_location)" @input="setValue($event)" class="form-control dv_common_select_for_all">
                                                    <option value="">Select </option>
                                        ...
                                        <div class="col-md-6 mb-3">
                                            <div class="form-group">
                                                ...
                                                <input name="date" v-model="saveStockTransfer.date" type="date" :class="status($v.saveStockTransfer.date)" @input="setValue($event)" class="form-control dv_common_input_for_all dvdatepicker">
                                            </div>                                                    
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <div class="form-group">
                                                <label class="text-muted text-capitalize" for="name">Contact Person</label>
                                                <input name="contact_person" v-model="saveStockTransfer.contact_person" type="text" :class="status($v.saveStockTransfer.contact_person)" @input="setValue($event)" class="form-control dv_common_input_for_all">
                                            </div>                                                    
                                        </div>
                                        <div class="col-md-6 mb-3">
                                            <div class="form-group">
                                                <label class="text-muted text-capitalize d-block" for="name">Transfer Stock Immediately? </label>
                                                <label class="ui-switch ui-switch-md info m-t-xs mt-2"><input checked="" v-model="saveStockTransfer.transfer_stock_immediately" type="checkbox"> <i></i></label>
                                            </div>                                                    
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-theme table-row v-middle">
                                    ...
                                    <tbody>
                                        <tr v-for="(item,index) in saveStockTransfer.products" :key="index" class="v-middle">
                                            <!-- <td>
                                    ...
                                    <div class="row">
                                        <div class="col-md-4 mb-3">
                                            <div class="form-group">
                                                ...
                                                <select v-model="saveStockTransfer.status" class="form-control">
                                                    <option value="">Select</option>
...
import { LOAD_PRODUCTS_LIST_ACTION, UPDATE_STOCK_TRANSFER_ACTION, GET_STOCK_TRANSFER_RECORD_ACTION  } from '@admin/store/products/actions';
...
export default {
    ...
    methods: {
        ...
        setValue: function(event) {
            this.saveStockTransfer[event.target.name] = event.target.value;
            this.$v.saveStockTransfer[event.target.name].$touch();
        },
        addProductInStock: function(){
            this.$v.current_product.$touch();
            if (!this.$v.current_product.$invalid) {
                this.saveStockTransfer.products.push({
                ...
                this.current_product.category_id = '';
        ...
        deleteProductInStock: function(product_id) {
            this.saveStockTransfer.products = this.saveStockTransfer.products.filter(product => product.product_id!=product_id);
        },
        ...
        updateStockTransfer: function () {
            let _this = this;
            _this.$v.saveStockTransfer.$touch();
            if (_this.saveStockTransfer.products.length == 0) {
                alert('Please add stock transfer atleast 1 product.');
                return;
            }
            let params = deleteEmptyKeys(this.saveStockTransfer);
            let products = [];
            ...
            params.stock_transfer_id = this.$route.params.stock_transfer_id;
            if (!_this.$v.saveStockTransfer.$invalid) {
        ...
        getStockTransferData: function() {
            this.$store.dispatch('getProduct/' + GET_STOCK_TRANSFER_RECORD_ACTION, this.$route.params.stock_transfer_id).then(() => {
                this.saveStockTransfer.from_location = this.stockTransferData.from_location;
                this.saveStockTransfer.to_location = this.stockTransferData.to_location;
                this.saveStockTransfer.date = this.stockTransferData.date;
                this.saveStockTransfer.contact_person = this.stockTransferData.contact_person;
                this.saveStockTransfer.transfer_stock_immediately = this.stockTransferData.transfer_stock_immediately;
                this.saveStockTransfer.status = this.stockTransferData.status;
                let products = this.stockTransferData.products;
                products.forEach(product => {
                    product.products = this.products;
                    product.brands = this.brands;
                });
                this.saveStockTransfer.products = products;
            });
        }
    }
}
