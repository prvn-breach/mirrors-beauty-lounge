    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">Product sale cost </h2>
                        ...
                        <table class="table table-theme table-row v-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 10%;" class="text-muted">no of orders</th>
                                    <th style="width: 6%;" class="text-muted">qty</th>
                            ...
                            <tbody>
                                <tr v-for="(product,index) in getProductSales.data" :key="index" class="v-middle">
                                    ...
                                    <td> {{ product['total_quantity'] ? product['total_quantity'] : 0 }}</td>
                                    <td>
                                        <strong>{{ product['total_price'] ? product['total_price'] : 0 }}</strong>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <pagination :data="getProductSales" @pagination-change-page="getProductSaleCostReport"></pagination>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    data() {
        return {
            ...
            excel_fields: [ 'Product ID', 'Product Name', 'No of Orders', 'Total Quantity', 'Total Price' ],
            excel_db_columns: [
                { column: 'id' },
                { column: 'name' },
                { column: 'total_orders' },
    ...
    methods: {
        getProductSaleCostReport: function(page = 1) {
            ...
            this.$store.dispatch('getReport/' + GET_PRODUCTS_SALE_COST_REPORT_ACTION, query);
        },
        excelDownload: function() {
            ...
            this.$store.dispatch('getReport/' + EXCEL_DOWNLOAD_PRODUCT_SALE_COST_REPORT_ACTION, params).then(() => {
                ...
                link.setAttribute("download", "ProductSaleCostReport.xlsx"); //or any other extension
