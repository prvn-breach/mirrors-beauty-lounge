    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">Most Active Customer Locations </h2>
                    <div class="row dv_search_delete_action_common">
                        <div class="col-md-5 col-sm-5 col-xs-5 col-5">
                            <input v-model="filters.search" @input="getActiveCustomerLocations" type="text" class="form-control dv_common_search_for_all" name="" placeholder="Search">
                        </div>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-3">
                                            <button type="button" class="btn btn-default dv_save_submit_btn" style="margin-top: 26px;" @click="getActiveCustomerLocations">Filter</button>
                                        </div>
                                    </div>
                                </div>
                            </div>                                    
                    ...
                    <div class="table-responsive">
                        <table class="table table-theme table-row v-middle">
                            <thead>
                                <tr>
                                    <th style="width: 30%;" class="text-muted">Location</th>
                                    <th style="width: 30%;" class="text-muted">Customers </th>
                                    <th style="width: 30%;" class="text-muted">Total Amount </th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(location,index) in getLocations" :key="index" class="v-middle">
...
import { GET_ACTIVE_CUSTOMER_LOCATIONS_ACTION, EXCEL_DOWNLOAD_ACTIVE_CUSTOMER_LOCATIONS_REPORT_ACTION  } from '@admin/store/reports/actions'
export default {
    components: { },
    data() {
        return {
    ...
    computed:{
        ...mapState({
            getLocations: state => state.getReport.getActiveCustomerLocations,
            excelLink: state => state.getReport.excelLink,
        })
    },
    mounted() {
        this.getActiveCustomerLocations();
    },
    methods: {
        getActiveCustomerLocations: function(page = 1) {
            let query = [];
            ...
            this.$store.dispatch('getReport/' + GET_ACTIVE_CUSTOMER_LOCATIONS_ACTION, query);
        },
        excelDownload: function() {
            ...
            this.$store.dispatch('getReport/' + EXCEL_DOWNLOAD_ACTIVE_CUSTOMER_LOCATIONS_REPORT_ACTION, query).then(() => {
                const link = document.createElement("a");
                link.href = this.excelLink;
                link.setAttribute("download", "MostActiveCustomerLocations.xlsx"); //or any other extension
