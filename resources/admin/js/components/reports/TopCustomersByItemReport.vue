    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">Top 10 Customers(Item) </h2>
                    <div class="row dv_search_delete_action_common">
                        <div class="col-md-5 col-sm-5 col-xs-5 col-5">
                            <!-- <input v-model="filters.search" @input="getCustomerSaleItemReport" type="text" class="form-control dv_common_search_for_all" name="" placeholder="Search"> -->
                        </div>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-3">
                                            <button type="button" class="btn btn-default dv_save_submit_btn" style="margin-top: 26px;" @click="getCustomerSaleItemReport">Filter</button>
                                        </div>
                                    </div>
                                </div>
                            </div>                                    
                    ...
                    <div class="table-responsive">
                        <table class="table table-theme table-row v-middle">
                            <thead>
                                <tr>
                                    <th style="width: 10%;" class="text-muted">customer id</th>
                                    <th style="width: 15%;" class="text-muted">customer</th>
                                    <th style="width: 15%;" class="text-muted">No. of items</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(customer,index) in getCustomers.data" :key="index" class="v-middle">
                                    ...
                                    <td>
                                        <strong>{{ customer['items_count'] }}</strong>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
import { GET_TOP_CUSTOMER_SALES_REPORT_ACTION, EXCEL_DOWNLOAD_CUSTOMER_SALES_REPORT_ACTION  } from '@admin/store/reports/actions'
export default {
    components: { },
    data() {
        return {
            ...
            excel_fields: [ 'Customer ID', 'Customer Name', 'No. of Items' ],
            excel_db_columns: [
                { column: 'user_id' },
    ...
    computed:{
        ...mapState({
            getCustomers: state => state.getReport.getTopCustomerSalesReport,
            excelLink: state => state.getReport.excelLink
        })
    },
    mounted() {
        this.getCustomerSaleItemReport();
    },
    methods: {
        getCustomerSaleItemReport: function(page = 1) {
            let query = [];
            ...
            this.$store.dispatch('getReport/' + GET_TOP_CUSTOMER_SALES_REPORT_ACTION, query);
        },
        excelDownload: function() {
            ...
            this.$store.dispatch('getReport/' + EXCEL_DOWNLOAD_CUSTOMER_SALES_REPORT_ACTION, params).then(() => {
                const link = document.createElement("a");
                link.href = this.excelLink;
                link.setAttribute("download", "TopCustomersByItemsReport.xlsx"); //or any other extension
