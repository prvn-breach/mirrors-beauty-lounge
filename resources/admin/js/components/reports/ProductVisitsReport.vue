    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">Product Visit Report </h2>
                        ...
                        <table class="table table-theme table-row v-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 25%;" class="text-muted">code </th>
                                    <th style="width: 15%;" class="text-muted">brand </th>
                                    <th style="width: 15%;" class="text-muted">category </th>
                                    <th style="width: 10%;" class="text-muted">views</th>
                                    <th style="width: 10%;" class="text-muted">year</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(product,index) in getProducts.data" :key="index" class="v-middle">
                                    ...
                                    <td>{{ product['product']['get_brand'] ? product['product']['get_brand']['name']: '' }}</td>
                                    <td>{{ product['product']['get_category'] ? product['product']['get_category']['name']: '' }}</td>
                                    <td>{{ product['count'] }} </td>
                                    <td>{{ new Date(product['product']['created_at']).getFullYear() }} </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
import { GET_PRODUCT_VISITS_REPORT_ACTION, EXCEL_DOWNLOAD_PRODUCT_VISITS_REPORT_ACTION  } from '@admin/store/reports/actions'
export default {
    components: { },
    data() {
        return {
    ...
    computed:{
        ...mapState({
            getProducts: state => state.getReport.getProductVisitsReport,
    ...
    methods: {
        getProductsData: function(page = 1) {
            ...
            this.$store.dispatch('getReport/' + GET_PRODUCT_VISITS_REPORT_ACTION, query);
        },
        excelDownload: function() {
            ...
            this.$store.dispatch('getReport/' + EXCEL_DOWNLOAD_PRODUCT_VISITS_REPORT_ACTION, query).then(() => {
                const link = document.createElement("a");
                link.href = this.excelLink;
                link.setAttribute("download", "ProductVisits.xlsx");
