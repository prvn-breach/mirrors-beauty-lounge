    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">SEM Expense Report </h2>
                    <div class="row dv_search_delete_action_common">
                        <div class="col-md-5 col-sm-5 col-xs-5 col-5">
                            <input @input="getSemExpenses" v-model="filters.search" type="text" class="form-control dv_common_search_for_all" name="" placeholder="Search">
                        </div>
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-3">
                                            <button type="button" class="btn btn-default dv_save_submit_btn" style="margin-top: 26px;" @click="getSemExpenses">Filter</button>
                                        </div>
                                    </div>
                                </div>
                            </div>                                    
                        ...
                        <table class="table table-theme table-row v-middle">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 12%;" class="text-muted">Reason </th>
                                    <th style="width: 12%;" class="text-muted">Added By </th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr v-for="(data,index) in getSemExpensesReport.data" :key="index" class="v-middle">
                                    <td>{{ data['from_date'] }} </td>
                                    <td>{{ data['to_date'] }} </td>
                                    <td>{{ data['type'] }} </td>
                                    <td>{{ data['type_name'] }} </td>
                                    <td><strong>{{ data['amount'] }} AED</strong></td>
                                    <td>{{ data['comments'] }} AED </td>
                                    <td>{{ data['created_by'] }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    <pagination :data="getSemExpensesReport" @pagination-change-page="getSemExpenses"></pagination>
                </div>
            </div>
        </div>
    </div>
...
import { GET_SEM_EXPENSES_REPORT_ACTION, EXCEL_DOWNLOAD_SEM_SALE_REPORT_ACTION } from '@admin/store/reports/actions';
export default {
    components: { },
    data() {
        return {
            ...
            excel_fields: [ 'ID', 'From Date', 'To Date', 'Type', 'Category / Brand', 'Spend Amount', 'Reason' ],
            excel_db_columns: [
               { column: 'id' },
               { column: 'from_date' },
               { column: 'to_date' },
    ...
    computed:{
        ...mapState({
            ...
            getSemExpensesReport: state => state.getReport.getSemExpensesReport
        })
    },
    mounted() {
        this.getBrandsList();
    ...
    methods: {
        ...
        getSemExpenses: function(page=1) {
            this.filters['page'] = page;
            let params = deleteEmptyKeys(this.filters);
            if (this.$admin['role_id'] == 2) {
                params['created_by'] = this.$admin['id'];
            }
            this.$store.dispatch('getReport/' + GET_SEM_EXPENSES_REPORT_ACTION, params);
        },
        excelDownload: function() {
            ...
            this.$store.dispatch('getReport/' + EXCEL_DOWNLOAD_SEM_SALE_REPORT_ACTION, params).then(() => {
                ...
                link.setAttribute("download", "SemExpensesReport.xlsx"); //or any other extension
