    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">Add Location </h2>
                        ...
                        <div class="col-md-12">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label class="text-muted text-capitalize" for="name">Location Type</label>
                                                <select class="form-control dv_common_select_for_all" v-model="addLocation.location_type" :class="status($v.addLocation.location_type)" @input="setValue($event)" name="location_type">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label class="text-muted text-capitalize" for="name">Start Time </label>
                                                <input type="time" v-model="addLocation.start_time" :class="status($v.addLocation.start_time)" @input="setValue($event)" name="start_time" class="form-control dv_common_input_for_all" placeholder="Enter">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label class="text-muted text-capitalize" for="name">End Time </label>
                                                <input type="time" v-model="addLocation.end_time" :class="status($v.addLocation.end_time)" @input="setValue($event)" name="end_time" class="form-control dv_common_input_for_all" placeholder="Enter">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group">
                                        <label class="text-muted text-capitalize" for="name">Latitude </label>
                                        <input v-model="addLocation.latitude" :class="status($v.addLocation.latitude)" @input="setValue($event)" name="latitude" type="text" class="form-control dv_common_input_for_all" placeholder="Enter">
                                    </div>
                                    <div class="form-group">
                                        <label class="text-muted text-capitalize" for="name">Longitude </label>
                                        <input v-model="addLocation.longitude" :class="status($v.addLocation.longitude)" @input="setValue($event)" name="longitude" type="text" class="form-control dv_common_input_for_all" placeholder="Enter">
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-12 text-center">
                            <button type="button" class="btn btn-default text-muted dv_cancel_submit_btn" @click="$router.push({ name: 'getLocations' })">cancel</button>
...
import { ADD_LOCATION_ACTION  } from '@admin/store/locations/actions'
export default {
    components: { },
    data() {
        return {
    ...
    methods: {
        ...
        saveLocation: function () {
            ...
            if (!_this.$v.addLocation.$invalid) {
                ...
                this.$store.dispatch('getLocation/' + ADD_LOCATION_ACTION, params).then(() => {
                    _this.loaderModal('hide');
                    this.$router.push({ name: 'getLocations' });
