    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">Products Never Purchase </h2>
                    <div class="row dv_search_delete_action_common">
                        <div class="col-md-5 col-sm-5 col-xs-5 col-5">
                            <input type="text" v-model="filters.search" @input="getProductsNeverPurchase" class="form-control dv_common_search_for_all" name="" placeholder="Search">
                        </div>
                        ...
                        <table class="table table-theme table-row v-middle">
                            ...
                            <tbody>
                                <tr v-for="(product,index) in getProducts.data" :key="index" class="v-middle">
                                    <td>{{ product['id'] }}</td> 
                                    <td>
                                        <img v-if="product['get_product_images'].length>0" :src="baseUrlImage+'images/products/'+product['get_product_images'][0]['image']" alt="" title="" class="dv_product_list_img">
                    ...
                    <pagination :data="getProducts" @pagination-change-page="getProductsNeverPurchase"></pagination>
                </div>
            </div>
        </div>
    </div>
...
import { GET_NEVER_PURCHASED_PRODUCTS_ACTION, EXCEL_DOWNLOAD_NEVER_PURCHASED_PRODUCTS_ACTION } from '@admin/store/orders/actions'
export default {
    data(){
        return {
    ...
    computed:{
        ...mapState({
            getProducts: state => state.getOrder.getProductsNeverPurchase,
            excelLink: state => state.getOrder.excelLink
        })
    },
    mounted() {
        this.getProductsNeverPurchase();
    },
    methods: {
        getProductsNeverPurchase: function (page=1){
            let query = [];
            ...
            this.$store.dispatch('getOrder/' + GET_NEVER_PURCHASED_PRODUCTS_ACTION, query);
        },
        excelDownload: function() {
            ...
            this.$store.dispatch('getOrder/' + EXCEL_DOWNLOAD_NEVER_PURCHASED_PRODUCTS_ACTION, query).then(() => {
                const link = document.createElement("a");
                link.href = this.excelLink;
                link.setAttribute("download", "ProductsNeverPurchase.xlsx"); //or any other extension
