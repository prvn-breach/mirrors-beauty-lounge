    ...
    <div id="content" class="flex">
        <div class="page-container">
            <div class="pt-3 pr-3 pb-5 pl-3">
                <div class="mb-5">
                    <h2 class="dv_page_heading">Current carts</h2>
                    <div class="row dv_search_delete_action_common">
                        <div class="col-md-5 col-sm-5 col-xs-5 col-5">
                            <input type="text" v-model="filters.search" @input="getCurrentCarts" class="form-control dv_common_search_for_all" name="" placeholder="Search">
                        </div>
                    ...
                    <pagination class="mt-2" :data="getCarts" @pagination-change-page="getCurrentCarts"></pagination>
                    <div class="table-responsive">
                        <table class="table table-theme table-row v-middle">
                            ...
                            <tbody>
                                <tr v-for="(cart, index) in getCarts.data ? getCarts.data.filter(cart => cart.user_id!=null) : []" :key="index" class="v-middle">
                                    <td>{{ cart['user']['id'] }} </td>
                                    <td>{{ cart['user']['name'] }} </td>
                                    <td><span class="item-except text-muted text-sm h-1x">
                                            {{ cart['user']['email'] }}
                                        </span></td>
                                    <td>{{ cart['cart_items_count'] }}</td>
                                    <td>
                                        <router-link :to="{ name : 'getCustomerDetail', params: { customer_id: cart['user']['id'], tab: 'cart' } }">
                    ...
                    <pagination :data="getCarts" @pagination-change-page="getCurrentCarts"></pagination>
                </div>
            </div>
        </div>
    </div>
...
import { GET_CURRENT_CARTS_ACTION, EXCEL_DOWNLOAD_CARTS_ACTION } from '@admin/store/orders/actions'
export default {
    data(){
        return {
    ...
    computed:{
        ...mapState({
            getCarts: state => state.getOrder.getCarts,
    ...
    methods: {
        getCurrentCarts: function (page=1){
            let query = [];
            ...
            this.$store.dispatch('getOrder/' + GET_CURRENT_CARTS_ACTION, query);
        },
        excelDownload: function() {
            ...
            this.$store.dispatch('getOrder/' + EXCEL_DOWNLOAD_CARTS_ACTION, query).then(() => {
                const link = document.createElement("a");
                link.href = this.excelLink;
                link.setAttribute("download", "CurrentCarts.xlsx"); //or any other extension
